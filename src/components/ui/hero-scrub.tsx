...
export function HeroScrub({
  ...
  // height difference reads as a misaligned mistake, not a deliberate
  // hand-off. Both converge on the SAME height here instead, flanking
  ...
  const foregroundLeftDelta = (icon?: HeroScrubIcon) => {
    if (!icon) return 0;
  ...
  const foregroundTopDelta = (icon?: HeroScrubIcon) => {
    if (!icon) return 0;
